...
func move_all_units(m_pos) -> void:
	var results = raycast_from_mouse(m_pos, 1)
	if results:
		get_tree().call_group("units", "move_to", results.position)
...
func get_unit_under_mouse(m_pos):
	var result = raycast_from_mouse(m_pos, 3) # collision mask 0...011
	if result and "team" in result.collider and result.collider.team == team:
		return result.collider
	
func get_units_in_box(top_left: Vector2, bot_right: Vector2):
